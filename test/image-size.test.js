...
test('setImageDimensions: http (good)', async t => {
  ...
  const url = 'http://78.media.tumblr.com/tumblr_lmighzVWof1qczr0io1_250.gif'
...
test('setImageDimensions: https @2x', async t => {
  const ImageSize = newImageSize()
  const url =
    'https://fulcrumtech.net/wp-content/themes/fulcrumtech/library/images/home/field-icons@2x.png'
...
test('setImageDimensions: http (bad domain)', async t => {
  const ImageSize = newImageSize()
  const url =
    'http://jasdfasdiennasdilnekendksdunaeucmnxliwndlie.asdfdedx/nope.gif'
  const $ = cheerio.load(`<img src="${url}">`)
  const result = await ImageSize.setAll($)
  const expected = `<img src="${url}">`
  t.deepEqual(result.html(), expected)
...
test('setImageDimensions: mixture', async t => {
  ...
  const html = `
        <img src="kitten.jpg">
        <img src="nope.jpg">
        <img src="http://78.media.tumblr.com/tumblr_lmighzVWof1qczr0io1_250.gif">
  ...
  const expected = `
        ...
        <img src="nope.jpg">
        <img src="http://78.media.tumblr.com/tumblr_lmighzVWof1qczr0io1_250.gif" width="250" height="129">
        <img src="http://24.media.tumblr.com/nope.gif">
    `
