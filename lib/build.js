...
Build.prototype.get = function() {
	...
	var handlebars = utils.requireAndInit('handlebars', this.config);
	var styles     = utils.requireAndInit('styles', this.config);
...
Build.prototype.generateDevHtml = function(html) {
	...
	var headPath = '../common/' + this.config.files.resetHead.replace('.scss', '.css');
	var headStylesheet   = stylesheet.clone().attr('href', headPath);

	var inlinePath = '../common/' + this.config.files.resetInline.replace('.scss', '.css');
	var inlineStylesheet = stylesheet.clone().attr('href', inlinePath);

	var stylesPath = 'css/' + this.config.files.styles.replace('.scss', '.css');
	var stylesStylesheet = stylesheet.clone().attr('href', stylesPath);
...
Build.prototype.saveEmails = function() {
	...
	var promises = [
		utils.saveEmail(this.html, this.config, 'html'),
		utils.saveEmail(this.htmlDev, this.config, 'dev.html'),
		utils.saveEmail(this.text, this.config, 'txt')
	];
