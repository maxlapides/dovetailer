...
Build.prototype.setImageDimensions = function($) {
    ...
    const getImageSize = src => {
        return _getImageSizeCache(src).then(cachedImgSize => {
    ...
    const _getImageSizeRelative = src => {
        ...
        return fse.accessAsync(filepath, fse.F_OK)
            .then(() => {
                const dimensions = sizeOf(filepath)
                this.addToImageCache(src, dimensions)
                return dimensions
            })
    }
...
Build.prototype.addToImageCache = function(src, dimensions) {
    ...
    const write = json => fse.outputJson(this.config.files.imageCache, json)

    fse.readJsonAsync(this.config.files.imageCache)
        .then(cache => {
            cache[src] = dimensions
            return write(cache)
        })
        .catch(err => write({ [src]: dimensions }))
}
