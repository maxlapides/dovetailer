...
HBSCompiler.prototype.registerPartials = function(partialsPath) {
    ...
    function _getRegisteredPromises(files) {
        return files.reduce(function (result, file) {
            var filePromise = that.getFile(partialsPath, file).then(function (contents) {
                handlebars.registerPartial(file.slice(0, file.indexOf('.')), contents);
            });
            result.push(filePromise);
            return result;
        }, []);
    }

    return partialsPathRead.then(function (files) {
        var promises = _getRegisteredPromises(files);
        return Promise.all(promises);
    }).catch(function(err) {
