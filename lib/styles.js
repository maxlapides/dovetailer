...
styles.prototype.compileResetStyles = function(styleName) {
    ...
    function _compile(resetPath, savePath) {
    ...
    return fse.accessAsync(resetPath)
        ...
        .then(function() {
            const savePath = getSavePath(that.config.dirs.build, that.tplName)
        ...
        .catch(function() {
            ...
            const savePath = getSavePath(that.config.dirs.build)
...
styles.prototype.compileSass = function(sassPath, savePath) {
    ...
    return sass.renderAsync(sassOpts)
        ...
        .then(postcssOutput => {
            fse.ensureFileAsync(saveFilename)
                .then(() => fse.writeFileAsync(saveFilename, postcssOutput.css))
