...
class NunjucksCompiler {
  ...
  cleanContext(data, markdownEnabled = false) {
    ...
    if (_.isString(data)) {
      ...
      return markdownEnabled ? marked(cleanData, { renderer }) : cleanData
    }

    // array
    if (_.isArray(data)) {
      return data.map(item => this.cleanContext(item, markdownEnabled))
    }

    // object
    return Object.entries(data).reduce((out, [key, val]) => {
      out[key] = this.cleanContext(val, markdownEnabled)
      return out
    }, {})
  }
}
