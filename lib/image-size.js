...
class ImageSize {
    ...
    const promise = new Promise((resolve, reject) => {
      get(src, response => {
        if (response.statusCode !== 200) {
          ...
          const error = `Could not find image ${src} (status code ${
            response.statusCode
          }).`
          logger.warn(error)
  ...
  copyLocalImgToBuild(src) {
    const from = path.resolve(this.tplPath, src)
    const to = path.resolve(config.dirs.build, this.tplName, src)

    // copy to build if it hasn't already been copied
    if (!copiedImages.includes(to)) {
      logger.warn(
        `References to local images will not work in production. Please copy ${src} to a server.`
      )
  ...
  addToImageCache(src, dimensions) {
  ...
  getCache() {
    ...
    return fse
      .readJsonAsync(config.files.imageCache)
      .then(cache => {
        imgCache = cache
        return imgCache
      })
...
const retinaWarning = (imgName, { width, height }, mod) => {
  if (width % mod !== 0) {
    logger.warn(
      `${imgName} is a retina image, but its width ${width}px is not divisible by ${mod}`
    )
  }
  if (height % mod !== 0) {
    logger.warn(
      `${imgName} is a retina image, but its height ${height}px is not divisible by ${mod}`
    )
  }
}
